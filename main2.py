...
#ブレンドが無いため汎化性能に欠ける。half_and_halfも二種のブレンド
#モデルによってbuilding_idが存在しない場合があり、それによって精度が下がっている可能性が大きい
#やはりbuilding_idごとにモデルを作る必要があるかもしれないが、分けると汎化性能が出ないため
#似たような値が出るビルについてそれぞれモデルを作ったほうが良いかもしれない
#また、trainとtestのどちらかにしか存在しないbuilding_idについても確認が必要
...
#%%
pred = np.expm1(model_half_1.predict(test_split_1, num_iteration=model_half_1.best_iteration))
pred = np.append(pred, np.expm1(model_half_2.predict(test_split_2, num_iteration=model_half_2.best_iteration)))
pred = np.append(pred, np.expm1(model_half_1.predict(test_split_3, num_iteration=model_half_1.best_iteration)))
pred = np.append(pred, np.expm1(model_half_2.predict(test_split_4, num_iteration=model_half_2.best_iteration)))
