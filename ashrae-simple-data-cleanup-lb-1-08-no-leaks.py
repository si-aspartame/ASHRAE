...
X_half_1 = X[:int(X.shape[0] / 2)]#split features in half
X_half_2 = X[int(X.shape[0] / 2):]

y_half_1 = y[:int(X.shape[0] / 2)]#split labels in half
y_half_2 = y[int(X.shape[0] / 2):]

model_1 = CatSplitRegressor(
    LGBMWrapper(random_state=0, n_jobs=-1, categorical_feature=categorical_columns), "meter")
model_1.fit(X_half_1, y_half_1)

model_2 = CatSplitRegressor(
    LGBMWrapper(random_state=0, n_jobs=-1, categorical_feature=categorical_columns), "meter")
model_2.fit(X_half_2, y_half_2)
...
test = compress_dataframe(_add_time_features(test))
test = test.drop(columns="timestamp")  # Raw timestamp doesn't help when prediction

#%%
pred = np.clip(np.expm1(model_1.predict(test)), 0, a_max=None) / 2
pred += np.clip(np.expm1(model_2.predict(test)), 0, a_max=None) / 2
